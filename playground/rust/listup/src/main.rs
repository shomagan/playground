    ...
    loop {
        ...
        match socket.recv_from(&mut buffer) {
            Ok((size, addr)) => {
                ...
                if let Some(json_content) = extract_json(data) {
                    debug!("Extracted JSON: {} from {}", json_content, addr);
                    if let Ok(data_json) = serde_json::from_str::<Value>(&json_content) {
            ...
            if let Some(result) = send_json(&client, addr, &read_request) {
                if let Some(value) = result.get("value").and_then(|v| v.as_f64()) {
                    if Some(value) != space.value {
                        ...
                        let write_result = send_json(&client, addr, &write_request);
                    }
                }
            }

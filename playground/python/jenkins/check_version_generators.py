...
from git import Git
__description__ = 'check version for module sofi plc'
...
def main():
    ...
    with Git().custom_environment(GIT_SSH_COMMAND=git_ssh_cmd):
        repo = Repo(last_version.sofi_plc_project_path+'/free_rtos/tools/generators')
        ...
        if tags_local[-1] != tags_remote[-1]:
            result = "changed"
            send_message("generators version changed")
        else:
            send_message("generators have the same version")
